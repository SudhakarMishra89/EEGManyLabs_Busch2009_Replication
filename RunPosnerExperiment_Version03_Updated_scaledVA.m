function RunExperiment(Name)
    ...
    defaults={'Testing','S01', '30', 'M', '53', '30', '75'};
 ...
 try
    ...
    trialnum = 0;
    nBlocks = 200; %rs commented from 100 to 75
    Trials = [];
    ...
    ecc =7.12; % VA scaled by 1.25 %1.25*5.7deg
    ...
    cross_arm_length = 20.55; %VA scaled by 1.25- 16.4 changed to 20.55
    cross_gap = 51.65; %VA scaled by 1.25- 40.75 changed to 51.65
    cross_color = [35 35 35];
    line_width = 2.5; %VA scaled by 1.25- 2 changed to 2.5
    ...
    % r&s changed from 3.5 to 7.8
    left_target_coords = [...
            LeftEccentricity-4.37 * PixelsPerDegree/60 ... %VA scaled by 1.25- 3.5 changed to 4.37
            centery-4.37 * PixelsPerDegree/60 ...
            LeftEccentricity+4.37 * PixelsPerDegree/60 ...
            centery+4.37 * PixelsPerDegree/60
    ];
    right_target_coords = [...
            RightEccentricity-4.37 * PixelsPerDegree/60 ...
            centery-4.37 * PixelsPerDegree/60 ...
            RightEccentricity+4.37 * PixelsPerDegree/60 ...
            centery+4.37 * PixelsPerDegree/60
    ];
    ...
    Screen( 'TextSize', FixCross, 35 );  % %VA scaled by 1.25- 28 changed to 35 %needed?
    FixString = '+';
    ...
    Screen( 'DrawText', FixCross, FixString, (x-11), (y-8), [25, 25, 25] ); %r&s changed i to get the fixation in the center approximately
    ...
    Screen( 'TextSize', LeftCueScreen, 9); % r&s changed from 9 to 20.25
    leftcuebmp = imread('ArrowLeft.bmp');
    leftcuebmp = leftcuebmp .* DisplayLuminance;
    leftcuebmp = imresize(leftcuebmp,0.16); %VA scaled by 1.25- 0.13 changed to 0.16
    ...
    rightcuebmp = imresize(rightcuebmp,0.16); %VA scaled by 1.25- 0.13 changed to 0.16
    ...
    for b = 1:nBlocks 
         %custom break added after 75 blocks -added by r&s
         if b==75
            Screen(window,'FillRect', [ 0, 0, 0 ]);
            Screen(window,'TextSize',16);
            DrawFormattedText(window, ['You have completed 75 blocks. Please call the experimenter.\n \n \n'  ' After the break, please press any key to begin.'],centerx*0.75,centery, [26, 26, 26] ); %rs changed the value from [6, 6, 6] to [16, 16, 16]
            %Screen(window,'DrawText', ['You have completed 75 blocks. Please call the experimenter.'  ' After the break, please press any key to begin'],centerx*0.6,centery, [26, 26, 26] ); %rs changed the value from [6, 6, 6] to [16, 16, 16]
            Screen('Flip', window);
            KbWait;
         end
         ShowCue2;       
