function RunExperiment(Name)
 ...
 try
    ...
    cross_arm_length = 16.4; %rs changed to 22.5 from 10 %to keep VA constant for 75cm viewing (scaled by 75/46)
    cross_gap = 40.75; %rs changed to 56.25 from 25 %VA %to keep VA constant for 75cm viewing (scaled by 75/46)
    cross_color = [35 35 35];
    line_width = 2; %rs changed to 3 from 1 %VA 
    ...
    Screen( 'TextSize', FixCross, 28 );  % r&s changed from 7 to 28 %va DOUBLING WAS NOT ENOUGH
    FixString = '+';
    ...
    leftcuebmp = imresize(leftcuebmp,0.13); % %to keep VA constant for 75cm viewing (scaled by 75/46) from 0.08 to 0.13
    ...
    rightcuebmp = imresize(rightcuebmp,0.13); % r&s changed from 0.08 to 0.18  %va DOUBLED
